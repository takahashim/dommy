...
実装済みの契約は本文の v1 案ではなく **flat v2** である。正は次の 3 点で、
本文の v1 設計は「span 形式が必要になったときの将来案」として残す。

- **emitter**: `Dommy::Rack::Trace::Ndjson`（1 行 1 JSON、`seq` が正順、
  op 固有フィールドは行に平坦化）
- **golden fixture**: `gems/dommy-rack/test/fixtures/contract.trace.ndjson` —
...
時間幅の表現（span）は v2 に無い。Rails 内部計装（trace-roadmap.md
フェーズ 2）で入れ子が必要になった時点で、`parent` + `duration_ms` を持つ
完成形 span 行を最小拡張として追加する（本文 v1 の span_start/end 分割は
同期実行の dommy には過剰、の判断も trace-roadmap.md に記録済み）。

---

以下は当初の v1 設計案（将来 span 形式を導入する際の参照資料）。
