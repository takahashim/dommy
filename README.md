...
A pure-Ruby, browser-like DOM stack for testing Rack/Rails apps without a real
browser. This repository is a monorepo containing the core library and its
integration gems.
...
Each gem keeps its own `gemspec`, `README`, and test suite, and is released to
RubyGems independently.
...
A single root `Gemfile` resolves all gems from their local paths, so no
`path:` overrides are needed:
...
`dommy` and `dommy-rack` use Minitest; `capybara-dommy` uses RSpec (including the
Capybara driver compliance suite).
