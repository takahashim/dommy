...
- Promise rejection tracking is whole. `event.reason` is the value the page rejected with, `event.promise` is the promise, and `rejectionhandled` fires when a handler arrives after a rejection was reported. All three needed the engine to hand the values over instead of converting them first: with one that cannot, rejections still arrive through the old route, with a reason the page can see but not read. A report the page recovers from is withdrawn, so it stops failing anything, while the console keeps the line it printed.
- A `<script>` whose evaluation throws now hands the page a real `Error` as `event.error`, with the same name, message and stack it threw. It used to get an opaque object with no properties at all, because the engine discards the JS value when it raises a host exception for the throw. That was not only missing information: a handler reading `e.error.message` threw on `undefined` and so never reached its own `preventDefault()`, leaving the report it meant to cancel standing. The rebuilt error matches on everything observable except identity (`e.error === thrown` is false), which needs the engine to hand the value over rather than convert it.
- `ErrorEvent`'s `filename` / `lineno` / `colno` are filled in, so `window.onerror` says where the page failed instead of reporting 0:0. The position comes from the JS frames on the thrown error; a callback's throw now carries its stack across the bridge, since the thrown value itself crosses as an opaque handle. Source the engine had no name for (an inline `<script>`, an `eval`) reports the document's URL, as a browser does, and Dommy's own bridge frames are never reported as the page's position.
- A dynamically inserted `<script src>` that downloaded fine and then threw now fires `load` on the element, not `error`. The element's `error` event means the fetch failed, and conflating the two tells a chunk loader (webpack, Vite) the network failed for a chunk that is sitting right there. The evaluation's exception is still reported at the global.
- Every uncaught JavaScript error now goes through WHATWG's "report an exception": it fires a cancelable `error` event at the window, and only an error the page leaves unhandled reaches the host's log. A page that cancels the event in `window.onerror` (or in an `unhandledrejection` listener) suppresses the failure, exactly as it silences a browser console. Previously a throwing `<script>` never reached `window.onerror` at all, a listener's exception fired the event but ignored whether the page canceled it, and `unhandledrejection` was never dispatched. `reportError()` is wired to the same algorithm instead of being swallowed.
- An exception thrown by a `setTimeout` / `setInterval` / `requestAnimationFrame` callback used to be swallowed by the JS bridge and disappeared entirely, not even reaching `js_errors`. It is now reported at the window like any other task's exception, and the timer keeps its schedule (an interval whose callback throws goes on firing, as in a browser).
- **Breaking:** `Dommy::Browser::JsError` is now `Dommy::JsError`, with no alias for the old name. The error's message also names the page's URL. The errors themselves live in a shared `Dommy::Js::ErrorLog`, reachable as `Browser#error_log`; `Browser#js_errors` still returns the same list.
...
- The URL setters follow the URL Standard: `protocol`, `host`, `hostname`, `port`, `pathname`, `search` and `hash` run the basic URL parser from the component's own state into the URL, so a value the parser rejects changes nothing, a scheme cannot move a URL between special and non-special, tabs and newlines are dropped, and `port = "8000/x"` keeps the digits. `<a>` and `<area>` have the same setters, plus `username` and `password`, and read their URL against the document base URL, which honours `<base href>`.
- A URL's host follows the URL Standard's domain parser: an ASCII domain is lowercased and checked for forbidden code points, whatever UTS #46 would make of it, so `https://xn--/` and an A-label that does not decode are hosts, as in browsers; a non-ASCII domain goes through ToASCII. The IDNA tables are built from Unicode 17.0, and a label that starts with any combining mark (General_Category Mark, not only the nonspacing ones) is rejected.
- A URL the parser rejects is refused where the specs say so: `XMLHttpRequest.open` and `window.open` throw a `SyntaxError` DOMException, `navigator.sendBeacon` a `TypeError`, and setting `location.href` (or `window.location`, `assign`, `replace`) a `SyntaxError`, while following a hyperlink to one does nothing. Those APIs resolve their URL with the URL parser against the document base URL instead of Ruby's URI.
