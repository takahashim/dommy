    ...
    module CSS
      ...
      module MediaQuery
        ...
        def match?(text, environment = DEFAULT)
          Evaluator.new(environment).match_list?(text)
        end

        # Tokenizing and value comparison / parsing depend on nothing but
        # their arguments. Mixed into Evaluator; they have no environment of
        # their own, so they stay a separate, stateless module.
        module Syntax
          module_function
        ...
        # One evaluation bound to a single Environment snapshot. The snapshot
        # is only as long-lived as the `match?` call that made this evaluator:
        # Environment is mutable (Window#resize_to rewrites it), so holding one
        # across evaluations would evaluate against a stale viewport.
        class Evaluator
          include Syntax
          ...
          # A comma-separated query list ORs together; the empty string is the
          # empty query list, which matches.
          def match_list?(text)
            ...
            # plain split is safe.
            text.split(",", -1).any? { |query| match_query?(query.strip) }
          end
          ...
          def match_query?(query)
            ...
            if first.is_a?(String)
              ...
              result = evaluate_condition(first[1])
            ...
            while i < tokens.size
              ...
              value = evaluate_condition(token[1])
          ...
          def evaluate_condition(inner)
            return nil if inner.empty?

            if inner.match?(/[<>]/) || (inner.include?("=") && !inner.include?(":"))
              evaluate_range(inner)
            elsif inner.include?(":")
              name, _, value = inner.partition(":")
              evaluate_feature(name.strip, value.strip)
            else
              evaluate_boolean_feature(inner)
            end
          end

          def evaluate_feature(name, value)
            case name
            ...
            when "aspect-ratio", "min-aspect-ratio", "max-aspect-ratio"
              evaluate_aspect_ratio(name, value)
          ...
          def evaluate_boolean_feature(name)
            case name
          ...
          def evaluate_aspect_ratio(name, value)
