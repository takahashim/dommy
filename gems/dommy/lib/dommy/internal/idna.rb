...
module Dommy
  module Internal
    ...
    # from Unicode 17.0 source files in `vendor/unicode/`).
    #
    ...
    module IDNA
      ...
      # UTS #46 V6: General_Category Mark (Mn, Mc or Me), which is wider than
      # the Bidi class NSM; a spacing mark such as U+1B44 is L.
      def self.validate_no_leading_combining_mark(label)
        first_cp = label.codepoints.first
        return unless first_cp
        return unless IDNAData.lookup(IDNAData::MARK, first_cp)
