...
module Dommy
  module Rack
    ...
    #
    # Only `visible?` is public; the helpers below it are implementation
    # detail and stay out of the `Dommy::Rack` namespace's own surface.
    module Visibility
      module_function
      ...
      private_class_method :hidden_by_closed_details?, :hidden_node?
    end

    # `Dommy::Rack.visible?(element)` stays the public entry point (used by
    # capybara-dommy and the test/debug helpers); the internal helpers no
    # longer ride along on the `Dommy::Rack` namespace itself.
    def self.visible?(element) = Visibility.visible?(element)
  end
end
