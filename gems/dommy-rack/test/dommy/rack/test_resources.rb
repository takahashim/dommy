...
class Dommy::Rack::TestResources < Minitest::Test
  ...
  def test_prefetch_warms_the_cache_so_a_later_get_skips_the_network
    ...
    # `:open` (browser mode) enables the concurrent prewarm.
    session = Dommy::Rack::Session.new(counting, cross_origin_subresources: :open)
  ...
  def test_prefetch_is_a_noop_outside_browser_mode
    res = resources # default session (:same_origin) — not browser mode
    res.prefetch(["/api"])
